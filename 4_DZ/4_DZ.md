...
Например, для создания глобального вторичного индекса используйте:
```
...
Пример использования транзакций в Java SDK:
```
cluster.transactions().run((ctx) -> {
    QueryResult result = ctx.query("SELECT * FROM inventory WHERE type = 'hotel'");
    // ... логика транзакции ...
});
```
